...
#include "itbf-queue-disc.h"

namespace ns3 {

NS_LOG_COMPONENT_DEFINE ("ITbfQueueDisc");

NS_OBJECT_ENSURE_REGISTERED (ITbfQueueDisc);

TypeId ITbfQueueDisc::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::ITbfQueueDisc")
    .SetParent<QueueDisc> ()
    .SetGroupName ("TrafficControl")
    .AddConstructor<ITbfQueueDisc> ()
    ...
    .AddAttribute ("PeakRate",
                   ...
                   MakeDataRateAccessor (&ITbfQueueDisc::SetPeakRate),
                   MakeDataRateChecker ())
    .AddAttribute ("MasterRun", 
                  "A daughter queue can call a run on the root queue with it ",
                   CallbackValue (),
                  MakeCallbackAccessor(&ITbfQueueDisc::m_masterRun),
                  MakeCallbackChecker())
    .AddTraceSource ("TokensInFirstBucket",
                     "Number of First Bucket Tokens in bytes",
                     MakeTraceSourceAccessor (&ITbfQueueDisc::m_btokens),
                     "ns3::TracedValueCallback::Uint32")
    .AddTraceSource ("TokensInSecondBucket",
                     "Number of Second Bucket Tokens in bytes",
                     MakeTraceSourceAccessor (&ITbfQueueDisc::m_ptokens),
                     "ns3::TracedValueCallback::Uint32")
  ;
...
void
ITbfQueueDisc::SetBurst (uint32_t burst)
{
...
uint32_t
ITbfQueueDisc::GetBurst (void) const
{
  NS_LOG_FUNCTION (this);
  return m_burst;
}
...
void
ITbfQueueDisc::SetMtu (uint32_t mtu)
{
...
uint32_t
ITbfQueueDisc::GetMtu (void) const
{
  NS_LOG_FUNCTION (this);
  return m_mtu;
}
...
void
ITbfQueueDisc::SetRate (DataRate rate)
{
...
DataRate
ITbfQueueDisc::GetRate (void) const
{
  NS_LOG_FUNCTION (this);
  return m_rate;
}
...
void
ITbfQueueDisc::SetPeakRate (DataRate peakRate)
{
...
DataRate
ITbfQueueDisc::GetPeakRate (void) const
{
  NS_LOG_FUNCTION (this);
  return m_peakRate;
}
...
uint32_t
ITbfQueueDisc::GetFirstBucketTokens (void) const
{
  NS_LOG_FUNCTION (this);
  return m_btokens;
}
...
uint32_t
ITbfQueueDisc::GetSecondBucketTokens (void) const
{
  NS_LOG_FUNCTION (this);
  return m_ptokens;
}
...
bool
ITbfQueueDisc::DoEnqueue (Ptr<QueueDiscItem> item)
{
      ...
      if (m_id.IsExpired () == true)
        {
          ...
          m_id = Simulator::Schedule (requiredDelayTime, &ITbfQueueDisc::ModifiedRun, this);
...
bool
ITbfQueueDisc::CheckConfig (void)
{
  NS_LOG_FUNCTION (this);
  if (GetNInternalQueues () > 0)
    {
      NS_LOG_ERROR ("ITbfQueueDisc cannot have internal queues");
      return false;
    }

  if (GetNPacketFilters () > 0)
    {
      NS_LOG_ERROR ("ITbfQueueDisc cannot have packet filters");
      return false;
    }
  ...
  if (GetNQueueDiscClasses () == 0)
    {
      ...
      if (!qd->SetMaxSize (GetMaxSize ()))
        {
          NS_LOG_ERROR ("Cannot set the max size of the child queue disc to that of ITbfQueueDisc");
          return false;
        }
      qd->Initialize ();
  ...
  if (GetNQueueDiscClasses () != 1)
    {
      NS_LOG_ERROR ("ITbfQueueDisc needs 1 child queue disc");
      return false;
    }
...
void
ITbfQueueDisc::InitializeParams (void)
{
  NS_LOG_FUNCTION (this);
