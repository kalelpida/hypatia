...
#ifndef POINT_TO_POINT_CHANNEL_TRACEN_H
#define POINT_TO_POINT_CHANNEL_TRACEN_H
 ...
 * \see TransmitStart
 */
class PointToPointTracenChannel : public Channel 
{
public:
  /**
  ...
  /**
   * \brief Create a PointToPointTracenChannel
   *
   * By default, you get a channel that has an "infinitely" fast 
   * transmission speed and zero delay.
   */
  PointToPointTracenChannel ();

  /**
   * \brief Attach a given netdevice to this channel
   * \param device pointer to the netdevice to attach to the channel
   */
  void Attach (Ptr<PointToPointTracenNetDevice> device);

  /**
   * \brief Transmit a packet over this channel
   * \param p Packet to transmit
   * \param src Source PointToPointTracenNetDevice
   * \param txTime Transmit time to apply
   * \returns true if successful (currently always true)
   */
  virtual bool TransmitStart (Ptr<const Packet> p, Ptr<PointToPointTracenNetDevice> src, Time txTime);
  ...
  /**
   * \brief Get PointToPointTracenNetDevice corresponding to index i on this channel
   * \param i Index number of the device requested
   * \returns Ptr to PointToPointTracenNetDevice requested
   */
  Ptr<PointToPointTracenNetDevice> GetPointToPointDevice (std::size_t i) const;
...
protected:
   ...
   * \returns Ptr to PointToPointTracenNetDevice source for the 
   * specified link
   */
  Ptr<PointToPointTracenNetDevice> GetSource (uint32_t i) const;

  /**
   * \brief Get the net-device destination
   * \param i the link requested
   * \returns Ptr to PointToPointTracenNetDevice destination for 
   * the specified link
   */
  Ptr<PointToPointTracenNetDevice> GetDestination (uint32_t i) const;
...
private:
  ...
  /**
   * \brief Wire model for the PointToPointTracenChannel
   */
  class Link
  {
public:
    ...
    Ptr<PointToPointTracenNetDevice> m_src;   //!< First NetDevice
    Ptr<PointToPointTracenNetDevice> m_dst;   //!< Second NetDevice
  };
